...
ATM_ARCHIVE="atm_${ATM_VERSION}_x86_64-unknown-linux-gnu.tar.gz"
ATM_REPO=randlee/atm-core
HERDR_VERSION=v0.8.2
HERDR_ARCHIVE=herdr-linux-x86_64
HERDR_REPO=herdrdev/herdr
HERDR_SHA256_PREFIX=976150a1   # v0.8.2 release sha256 prefix (verified)
HERMES_ATM_WHEEL="$WHEELHOUSE/hermes_atm-1.4.2-py3-none-any.whl"
ATM_GRAFT_WHEEL=atm_graft-1.4.3-cp311-abi3-manylinux_2_17_x86_64.manylinux2014_x86_64.whl
ATM_GRAFT_URL="https://test.pypi.org/packages/$(curl -fsSL https://test.pypi.org/pypi/atm-graft/1.4.3/json | python3 -c "import json,sys; d=json.load(sys.stdin); print([u['url'] for u in d['urls'] if 'manylinux_2_17_x86_64' in u['filename']][0].split('/packages/')[1])")"
...
fetch_assets() {
  ...
  if [ -n "${ATM_TARBALL:-}" ]; then
    ...
    ATM_VERSION=$(echo "$ATM_ARCHIVE" | sed 's/atm_\(.*\)_x86_64.*/\1/')
...
build_testbed() {
  ...
  cd "$HERE"
  cd assets
  ATM_TARBALL_NAME="$(ls atm_*_x86_64-unknown-linux-gnu.tar.gz | head -1)"
